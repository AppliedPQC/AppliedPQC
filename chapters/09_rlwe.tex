...
The tool that realizes this $O(n\log n)$ multiplication is the \emph{number-theoretic transform} (NTT), the finite-field analogue of the FFT: where the FFT works over the complex numbers, the NTT works over $\mathbb{Z}_q$, turning the cyclic convolution of two polynomials into cheap pointwise multiplication. The NTT is central enough to ML-KEM that it gets a full chapter of its own: Chapter~\ref{ch:ntt} builds it from first principles, including the negacyclic variant required by the ring $\mathbb{Z}_q[x]/(x^n+1)$ used here. For now it is enough to know that such a fast polynomial multiplication exists.
...
\noindent where the RLWE step is precisely the move to a polynomial ring with cyclic (negacyclic) structure.
