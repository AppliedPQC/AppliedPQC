...
The tool that realizes this $O(n\log n)$ multiplication is the \emph{number-theoretic transform} (NTT) of Chapter~\ref{ch:ntt}: the finite-field analogue of the FFT, it works over $\mathbb{Z}_q$ and turns the cyclic convolution of two polynomials into cheap pointwise multiplication, in the negacyclic form required by the ring $\mathbb{Z}_q[x]/(x^n+1)$ used here. It is precisely this transform -- built earlier as a foundational tool -- that makes RLWE's polynomial products fast enough for practice.
