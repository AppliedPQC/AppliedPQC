...
ML-DSA in the previous chapter rested on lattice problems. FIPS 205, \textbf{SLH-DSA} (from the submission SPHINCS\textsuperscript{+}), rests on nothing but hash functions. There is no number theory, no lattice, no algebraic structure an attacker could exploit -- if the hash function is secure, so is the signature. This makes it the most conservative post-quantum signature, chosen precisely as a hedge in case a structural weakness is ever found in lattices.
...
The atomic building block is a \emph{one-time} signature: a key pair that may safely sign exactly one message. WOTS\textsuperscript{+} (Winternitz) works by repeated hashing along chains.
...
A one-time key is nearly useless on its own. The Merkle tree fixes this: generate many WOTS\textsuperscript{+} key pairs, hash each public key into a leaf, and build a binary tree of hashes up to a single \emph{root}. That one root is the public key for \emph{all} the leaves at once.
