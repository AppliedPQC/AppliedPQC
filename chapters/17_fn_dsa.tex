...
\subsection{Where the signature bytes go}
\label{sec:falcon-sizes}

The sizes in the parameter table (Section~\ref{sec:falcon-params}) are not rounded estimates but sums of fixed fields, and each field follows from something already on the page. A Falcon signature is
\[
...
\]
giving $666$ and $1280$ bytes. The header byte records the encoding and $\log_2 n$; the salt is the 320-bit value hashed with the message. Everything that varies with the security level is in $\ell_n$, and $\ell_n$ is set by two facts: how wide the sampler's Gaussian is, and how the coefficients are coded.

\paragraph{The width.} The fast Fourier sampler outputs $(s_1,s_2)$ with each coefficient a discrete Gaussian of standard deviation
\[
\sigma \;=\; 1.17\,\sqrt{q}\;\eta_\varepsilon(\mathbb{Z}^{2n}),
\]
where $\eta_\varepsilon$ is the smoothing parameter of Section~\ref{sec:gpv} at the $\varepsilon$ the specification fixes; the specification tabulates $\eta_\varepsilon \approx 1.278$ for $n=512$ and $1.298$ for $n=1024$, so $\sigma \approx 165.7$ and $168.4$. The factor $1.17\sqrt{q}$ is the Gram--Schmidt quality that key generation guarantees for the secret basis (Section~\ref{sec:falcon-keygen}); a coefficient of $s_2$ is thus an integer of typical magnitude a hundred or two, out of a range of $q=12289$.

\paragraph{The code.} A coefficient stored as a raw residue costs $\lceil\log_2 q\rceil = 14$ bits, which would put $s_2$ alone at $896$ bytes for $n=512$. \textsc{Compress} instead writes each coefficient $c$ as one sign bit, the low seven bits of $|c|$ verbatim, and the remaining high part $\lfloor |c|/128\rfloor$ in unary ($k$ zero bits followed by a one). With $\sigma\approx166$, $|c|<128$ for about $56\%$ of coefficients and $|c|<256$ for about $88\%$, so the unary part averages roughly $0.6$ bits, and the expected cost is
\[
1 + 7 + (0.6 + 1) \;\approx\; 9.6\ \text{bits per coefficient},
\]
about $613$ bytes for $n=512$. The information-theoretic floor for a Gaussian of this width, $\log_2\bigl(\sigma\sqrt{2\pi e}\bigr)\approx 9.4$ bits, shows the code is within a few percent of optimal.

\paragraph{The fixed length.} A variable-length signature would leak the size of $s_2$ and complicate every parser downstream, so the specification fixes $\ell_n$ slightly above the mean: $625$ bytes for $n=512$ is $9.77$ bits per coefficient, $1239$ for $n=1024$ is $9.68$. An encoding that comes up short is padded with zero bits; the rare draw whose encoding would exceed $\ell_n$ is discarded and the signer samples again. That resampling is why the table's figures can be exact rather than typical.

The public key has no such structure to exploit. Its single polynomial $h$ has coefficients uniform modulo $q$, so it costs the full $14n/8$ bytes plus a header byte: $897$ for $n=512$ and $1793$ for $n=1024$. The asymmetry is the whole design in one line: the key is a random-looking ring element and must be stored as one, while the signature is a short Gaussian vector and is stored as one.
...
Analyses of Falcon's floating-point sensitivity make this implementation boundary
concrete: compiler, instruction, and rounding differences can perturb the sampled
distribution in ways relevant to secret-key security \cite{sleepingfalcon2025}.
...
\section{Research directions in compactness and acceleration}
\label{sec:fndsa-acceleration}

Sections~\ref{sec:gpv} and~\ref{sec:falcon-sizes} explain how Falcon's sampler and
encoding produce compact signatures. Recent analyses tighten GPV preimage sampling under
weaker smoothness conditions and use R\'enyi divergence to measure the gap between
Falcon's real and ideal samplers more precisely
\cite{gpvweak2025,falconcloser2024}. Other work combines probability coding,
sampling geometry, and modulus choices to shorten hash-and-sign lattice signatures
\cite{shorterhashsign2022}. These results can reduce proof slack or transport size,
but every saving must be reported together with its security margin and sampler
requirements.

MITAKA explores a different engineering point: it retains the NTRU hash-and-sign
direction while replacing Falcon's fast Fourier sampler with a hybrid sampler and
a stronger-quality trapdoor, producing a structure intended to be easier to
parallelize and mask \cite{mitaka2022}. The comparison is useful precisely because
MITAKA is a research construction rather than FN-DSA: simplifying an implementation
may require changing the primitive, not merely optimizing Falcon code.

Acceleration is equally workload-specific. FPGA and ASIC designs expose area,
memory-port, power, and energy costs hidden by software benchmarks
\cite{beckwithhw2022,beckwithhw2024}. GPU implementations can obtain very high
Falcon batch throughput; GOLF, for example, reports results on an RTX~4090 using
message-level parallelism, FFT kernels, and cache-locality optimization
\cite{golf2025}. Such figures do not predict the latency of one interactive
signature: batch size, transfer overhead, occupancy, tail latency, and tenant
isolation must be measured separately.
...
\bottomrule
\end{tabular}
\caption{The two FN-DSA (Falcon) parameter sets, both with $q=12289$. Signatures and keys are markedly smaller than ML-DSA's or SLH-DSA's -- the reason Falcon exists. Section~\ref{sec:falcon-sizes} derives the byte counts.}
\end{table}
