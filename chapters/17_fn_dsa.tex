...
The second lattice-based signature standard, FIPS 206, is \textbf{FN-DSA} -- the standardized form of Falcon~\cite{fips206,falcon2020}. (At the time of writing it is the newest of the four NIST standards, finalized after ML-KEM, ML-DSA, and SLH-DSA.) Where ML-DSA uses Fiat--Shamir with aborts, Falcon takes a different route on the same lattice foundation: \emph{hash-and-sign} over NTRU lattices, using a Gaussian trapdoor sampler. The payoff is remarkably compact signatures and keys; the price is the most delicate implementation of the three.
