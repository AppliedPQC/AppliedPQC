...
companion file \texttt{sage/fips206\_fndsa.sage} goes the rest of the way:
...
\begin{itemize}
  \item the NTRU solver runs over \texttt{ZZ['x']}, so the trapdoor equation
        ...
        point -- \texttt{verify\_ntru\_equation()} checks it as an identity
        in $\mathbb{Z}[x]/(x^n+1)$;
...
The function \texttt{verify\_fft\_against\_exact\_ring()} closes the loop
