...
for _ in range(10):
    ...
    print(A.rank())   # 4 (invertible) almost always over a field
\end{lstlisting}

Over a finite field a random square matrix is invertible with high probability, so a random $A$ almost always has full rank.
...
This efficiency is precisely why a plain linear system carries \emph{no} cryptographic value: anyone who sees $A$ and $\mathbf{b}=A\mathbf{s}$ can recover the secret $\mathbf{s}$ in cubic time. The entire idea of lattice cryptography is to break this by adding a small, structured error. Given samples
\[
\mathbf{b} = A\mathbf{s} + \mathbf{e},
\]
with $\mathbf{e}$ short, the equation is no longer exactly consistent, Gaussian elimination no longer applies, and recovering $\mathbf{s}$ becomes the \emph{Learning With Errors} problem (Chapter~\ref{ch:lwe}) -- believed hard even for quantum computers. The whole difficulty is manufactured by the single step of replacing an exact equation with an approximate one.

\begin{remark}
It is worth pausing on how little changes on the page and how much changes in difficulty. The map $\mathbf{s}\mapsto A\mathbf{s}$ is linear and invertible; the map $\mathbf{s}\mapsto A\mathbf{s}+\mathbf{e}$ is not even a function of $\mathbf{s}$ alone. Cryptographic hardness lives in that gap.
\end{remark}
...
A = random_matrix(F, 4, 4); v = random_vector(F, 4)
print(A * v)

# 3. exact solve is easy; try recovering s from b = A*s
s = random_vector(F, 4); b = A * s
print(A.solve_right(b) == s)   # True: Gaussian elimination recovers s

# 4. now perturb by a small error and watch exact recovery fail
e = vector(F, [1, 0, 16, 0])   # 16 == -1 mod 17
print(A.solve_right(b + e) == s)  # almost always False
\end{lstlisting}

Experiment~4 is the whole book in miniature: a one-line perturbation turns a solvable system into one whose exact solution no longer reveals the secret.
