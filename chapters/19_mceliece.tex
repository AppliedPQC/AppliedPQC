...
The whole of Parts II--V rested on one hard problem family: lattices. This part opens a second, entirely independent foundation. Code-based cryptography does not ask you to find short vectors in a lattice; it asks you to \emph{decode} a random-looking linear code. It is the oldest post-quantum family of all -- Robert McEliece proposed his scheme in 1978, the same year RSA appeared -- and after more than forty years of attack it stands essentially unbroken. That track record is precisely why it is prized: where the lattice standards trade a little conservatism for small keys and speed, Classic McEliece makes the opposite bet.
...
Classic McEliece builds its secret from a \textbf{binary Goppa code}. The details of Goppa codes are beyond this chapter, but the two properties we need are simple to state:
\begin{itemize}
...
We present two views of the same idea. The original 1978 form encrypts a message directly and is the clearest illustration of the trapdoor. The modern form -- the one the NIST ``Classic McEliece'' submission actually specifies -- is a key-encapsulation mechanism (KEM) built on the dual, parity-check description of the code. Both rely on the same secret decoder.
