...
full masking. One qualification has emerged: dynamic frequency scaling (Turbo Boost
and its relatives) makes the clock frequency depend on power draw, which depends on
the data being processed -- so power leakage can surface in \emph{remote} timings even
when the code itself is constant-time, and network key-extraction attacks over this
channel have been demonstrated~\cite{hertzbleed2022}. Where the threat model includes
them, disabling frequency boosting for cryptographic workloads is the direct
countermeasure. A smart card or an embedded key store, where the attacker holds the
...
The post-quantum mathematics of the previous chapters is genuine: it
