...
\begin{itemize}
  \item \textbf{Worst-case to average-case.} A scheme's keys are random (average-case) LWE instances. The reduction guarantees that breaking a random key is as hard as the worst conceivable lattice, so there are no ``weak keys'' to stumble into. Factoring offers no comparable guarantee. The discrete-logarithm problem in a fixed group, by contrast, does have a worst-case to average-case \emph{self}-reduction (randomize the target, solve, subtract the randomizer), and a tight one; what is distinctive here is not self-reduction as such but the passage from a \emph{different} problem, a worst-case lattice problem, to the average case.
...
\begin{remark}[What the reduction guarantees at deployed sizes]
The theorem is asymptotic. Unwinding it at concrete parameters costs polynomial
factors and approximation-factor inflation, and analyses that track those
constants find that the known reductions yield no meaningful security bound at
the dimensions standardized schemes actually use -- they begin to bind only at
dimensions far beyond $n=1024$~\cite{gartner2023}. The reduction is therefore
evidence about the LWE problem \emph{family}, not a security proof for any
deployed parameter set; this is why Section~\ref{sec:lwe-cryptanalysis} sets
concrete parameters against BKZ-based attack estimates rather than against
Theorem~\ref{thm:regev}.
\end{remark}
