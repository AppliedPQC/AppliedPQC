...
\begin{itemize}
  ...
  \item \textbf{A dual solution breaks the primal.} An SIS solution \emph{is} an LWE distinguisher. For a short $\mathbf{z}$ with $\mathbf{A}\mathbf{z}=\mathbf{0}$,
  \[
  ...
  \]
  which is \emph{small} because $\mathbf{z}$ and $\mathbf{e}$ are both short, whereas $\langle\mathbf{z},\mathbf{u}\rangle$ for a uniform $\mathbf{u}$ is spread over all of $\mathbb{Z}_q$. That gap decides Decision-LWE, and it is the engine of the dual attack of Section~\ref{sec:lwe-cryptanalysis}; conversely, an LWE (BDD) oracle yields short dual vectors, so each problem solves the other.
\end{itemize}

Both problems also reduce to worst-case lattice problems -- SIS to the worst case of SIVP~\cite{ajtai1996}, as LWE does through Theorem~\ref{thm:regev} -- which is why breaking either is hard for the same underlying reason. We will meet SIS again directly: in ML-DSA / Dilithium (Chapter~\ref{ch:mldsa}), forging a signature amounts to producing a short vector satisfying a public equation, which is precisely an SIS instance.
