...
The previous part of the book built up to FIPS 203, the real ML-KEM standard for \emph{encryption and key exchange}. We now cross to the other half of public-key cryptography, \emph{digital signatures}, and to the first of three signature standards: FIPS 204, \textbf{ML-DSA}, better known by its submission name Dilithium.
...
ML-DSA leans on two lattice problems in the module setting of Chapter~10.
...
\begin{itemize}
  ...
  \item \textbf{Module-SIS} (Short Integer Solution): given a random $A$, finding a short nonzero $\mathbf{z}$ with $A\mathbf{z}=\mathbf{0}\pmod q$ is hard. This blocks forgery: a forger would have to produce a short vector satisfying a public equation.
\end{itemize}
...
Signing is a three-move identification transcript made non-interactive by the Fiat--Shamir transform. Informally:

\begin{enumerate}
  \item \textbf{Commit.} Sample a random short masking vector $\mathbf{y}$ and form $\mathbf{w} = A\mathbf{y}$.
  \item \textbf{Challenge.} Compute a small challenge polynomial $c$ by hashing the commitment together with the message: $c = H(tr\,\|\,\textsc{HighBits}(\mathbf{w})\,\|\,M)$.
...
\begin{tikzpicture}[node distance=5.5mm and 11mm]
  ...
  \node[flownode, right=of w] (c) {$c=H(tr\,\|\,\textsc{HighBits}(\mathbf{w})\,\|\,M)$};
...
\begin{algorithmic}[1]
\Require Secret key $(\rho,K,tr,\mathbf{t}_0,\mathbf{s}_1,\mathbf{s}_2)$, message $M$
\Ensure Signature $\sigma=(c,\mathbf{z},\mathbf{h})$
\State $A \gets \textsc{ExpandA}(\rho)$;\quad $\mu \gets H(tr\,\|\,M)$
\State $\kappa \gets 0$;\quad $\rho'' \gets H(K\,\|\,\mu)$ \Comment{deterministic per-message randomness}
\Loop
  ...
  \State $c \gets H(\mu\,\|\,\mathbf{w}_1)$ \Comment{Fiat--Shamir challenge}
...
# ... build A, s1, s2, t = A*s1 + s2 as in the toy Kyber chapter ...
...
The number of iterations depends on the mask, which is fresh randomness, not on the secret in an exploitable way. FIPS 204 further derives the per-message randomness deterministically from $K$ and $M$, so the default signing mode is reproducible.
...
The next chapter turns to a completely different foundation for signatures -- FIPS 205, SLH-DSA, which needs no lattices or number theory at all, only hash functions.
