...
Figure~\ref{fig:labrador-round} writes one round out in the convention of the
...
$\{(a^{(k)}_{ij},\boldsymbol{\varphi}^{(k)}_i,b^{(k)})\}_k$; norm bound
...
binds only short inputs, so they are first split into low-norm digits and the
