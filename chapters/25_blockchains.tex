...
For Bitcoin, a signature is an economic object rather than only a cryptographic
one. Replacing a tens-of-bytes ECDSA or Schnorr signature with a kilobyte-scale
post-quantum signature changes transaction weight, fees, block capacity, storage,
relay, and synchronization. Falcon is attractive for compactness but has a more
demanding sampler and numerical implementation; ML-DSA is structurally simpler but
larger. The relevant comparison must therefore include both wire footprint and
implementation risk.
...
\section{Ethereum}
\subsection{What is exposed}
Ethereum has four distinct post-quantum migration surfaces: account authorization
based on secp256k1/ECDSA, BLS consensus signatures, KZG commitments for data
availability, and pairing- or discrete-log-based proof systems in applications and
rollups. A roadmap that replaces only account signatures therefore leaves other
quantum-vulnerable assumptions in the protocol stack.
...
\subsection{On-chain verification choices}
\label{sec:onchain}
...
Direct verification executes the post-quantum verifier in the contract. A
proof-based design instead proves that verification occurred and checks only the
proof on-chain. The latter changes both the cost model and the assumptions; the
Layer~2 composition issues are developed in Section~\ref{sec:l2-composition}, and
the post-quantum commitment choices inside such proofs in
Section~\ref{sec:pq-proof-systems}.
...
Research schemes illustrate why there is no generic ``threshold wrapper.''
Threshold Raccoon chooses a signing relation designed for collaborative
computation and masks participant responses so that the masks cancel only in the
aggregate \cite{thresholdraccoon2024}. Flood and Submerse tackles distributed key
generation and robust signing by combining projection, Gaussian noise, and
verifiable secret sharing to keep hidden shares within the correctness region
\cite{floodsubmerse2024}. In both cases, rounds, communication, robustness, and the
noise budget are part of the construction rather than implementation details.
...
A signature verifier that is fast in native code may still be expensive inside
an arithmetic circuit. Loquat, for example, builds a succinct verification
relation around the Legendre PRF and a multivariate polynomial commitment
\cite{loquat2024}. Evaluating such a route requires constraint count, field
compatibility, witness-generation time, proof size, verification cost, and the
maturity of its assumptions---not CPU cycles alone.
