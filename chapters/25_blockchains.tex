...
\begin{itemize}
  ...
  \item \textbf{Lattice-based} commitments --- the LaBRADOR
        line~\cite{labrador2023} --- commit with an Ajtai/Module-SIS commitment
        (Chapter~\ref{ch:mlwe}) that is opened by \emph{ring arithmetic} rather
        than by hashing. There is no per-level hash in the opening at all, which
        is the structural difference: a Merkle path costs a hash per level,
        while a module-lattice opening costs a multiplication in
        $\mathbb{Z}_q[X]/(X^d+1)$ --- an NTT, the transform of
        Chapter~\ref{ch:ntt}.
\end{itemize}

\paragraph{Smallest is not cheapest.}
Lattice arguments produce the smallest post-quantum proofs by a wide margin:
LaBRADOR proves an R1CS with $2^{20}$ constraints in \textbf{58\,kB} at the
128-bit level, against megabytes for the hash-based family. The cost has moved
rather than disappeared, and it has moved to the verifier. LaBRADOR's verifier
is \emph{linear} in the witness, and Greyhound --- the polynomial commitment
built on it --- measures \textbf{2.80\,s} to verify a degree-$2^{30}$ evaluation,
which its own authors put at roughly twice Ligero's~\cite{greyhound2024}. A
system that must verify inside a smart contract or a script is buying bytes with
time, and should confirm it wants that trade.
