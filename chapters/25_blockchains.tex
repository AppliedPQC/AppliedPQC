...
\subsubsection{LaBRADOR, in outline}
...
\]
one such equation per constraint. R1CS, and the verification equations of
lattice signatures, translate into this form; that is what makes it a
...
\begin{enumerate}
  \item \textbf{Commit.} The prover commits to each $\mathbf{s}_i$ with an
        ...
        all the $\mathbf{t}_i$, split into low-norm digits, so that what is sent
