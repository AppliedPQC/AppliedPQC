...
A proof system needs to commit to a \emph{polynomial} and later prove an
evaluation. The bridge is a square-root decomposition. Write
...
Both the transcript and the verifier are $O(\sqrt{N})$. Greyhound's contribution
is a three-round protocol of this shape with the norm control that lattices
require, and then a \emph{composition}: run LaBRADOR over the resulting statement
to compress the $O(\sqrt{N})$ transcript to polylogarithmic
size~\cite{greyhound2024}.
