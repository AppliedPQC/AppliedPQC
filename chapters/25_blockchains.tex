...
\paragraph{It compresses, in bytes and in security.}
The commitment is $\kappa$ ring elements no matter how large $n$ is, so
$\kappa d\lceil\log_2 q\rceil$ bits stand for a witness of any length. The
second half is easy to miss and is the better half: because the hardness of SIS
does not really depend on the \emph{dimension} of the solution, lengthening the
witness does not weaken the assumption either. A longer witness is free twice
over. And $\mathbf{A}$ is public randomness --- no trusted setup and no
...
\paragraph{Hiding is bought separately, and there are two ways to buy it.}
...
\]
The two trade against each other in a way worth knowing, because it explains the
shape of every protocol built on them. Ajtai demands a \emph{short} message, but
...
does not hand back the witness that was committed. What it yields is a
...
smallness~\cite{falconagg2024}. Removing slack is not polish; it is what a
decade of work in this area has largely been about.
