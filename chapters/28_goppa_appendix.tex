...
The condition is $\mathbb{F}_{2^m}$-linear in $c$, so $\Gamma(L,g)$ is a linear code; and although the constraints are written over $\mathbb{F}_{2^m}$, the codewords are restricted to be \emph{binary}. This restriction is exactly what makes the binary case special, as Section~\ref{sec:goppa-distance} shows.
...
\begin{algorithm}[H]
\caption{Patterson decoding of a binary Goppa code $\Gamma(L,g)$, $\deg g=t$ squarefree}
\label{alg:patterson}
\begin{algorithmic}[1]
\Require Received word $y\in\mathbb{F}_2^{\,n}$ with at most $t$ errors; secret $L$, $g$
...
\State $R(x) \gets \sqrt{\,T(x)+x\,} \bmod g(x)$ \Comment{unique square root mod $g$, as $g$ is squarefree}
...
F.<a> = GF(2^4)
PR.<x> = F[]
g = x^2 + x + a                     # squarefree: gcd(g, g') = gcd(g, 1) = 1
...
Here $mt = 4\cdot 2 = 8$, so the dimension satisfies $k\ge n-8$, and by Proposition~\ref{prop:goppa-double} the minimum distance is at least $2t+1=5$, so the code corrects $t=2$ errors. Scaling the same recipe to $m\approx 13$, $t\approx 100$ produces the codes Classic McEliece actually uses; the only thing that changes is the size.
