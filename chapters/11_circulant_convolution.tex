...
This chapter is foundational for everything that follows. The transition from a matrix to a polynomial, and from matrix multiplication to convolution, is exactly what makes RLWE and Module-LWE efficient, and it is what the NTT (Chapter~\ref{ch:ntt}) accelerates inside ML-KEM. The same algebra also underpins lattice-based fully homomorphic encryption and zero-knowledge proof systems -- topics beyond the scope of this book -- so the transition developed here is worth internalizing before those constructions can look anything but mysterious.
...
\begin{tikzpicture}[node distance=6mm and 6mm, every node/.style={font=\scriptsize}]
  \node[flownode] (a) {polynomial};
  \node[flownode, right=of a] (b) {circulant\\[-2pt]matrix};
  \node[flownode, right=of b] (c) {circular\\[-2pt]convolution};
  \node[keynode, right=of c] (d) {NTT pointwise\\[-2pt]multiplication};
  \draw[flowarrow] (a) -- (b);
  \draw[flowarrow] (b) -- (c);
  \draw[flowarrow] (c) -- (d);
\end{tikzpicture}
...
This is the hidden bridge that makes RLWE and Module-LWE practical. It is also the deep reason why the same algebra appears again and again across lattice-based cryptography, from Kyber to fully homomorphic encryption and beyond.
