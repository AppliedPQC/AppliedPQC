...
A category assignment is a claim, and claims can be contested. The Category 1
assignment of ML-KEM-512 in particular has been publicly disputed: the
underlying lattice attack-cost estimates carry tens of bits of stated
uncertainty~\cite{matzov2022}, and the calculation NIST offered in support of
the assignment has been challenged on the grounds that it combined computation
and memory-access costs incorrectly~\cite{bernstein2023kyber}. The dispute is
about that one parameter set, not about the category system; deployments have
largely standardized on ML-KEM-768 and above, which sidesteps it.
