...
Post-quantum security does not come from one idea but from \emph{five} independent families of hard problems, each a different bet against the quantum future~\cite{bernstein2009}. A scheme is only as trustworthy as the family it belongs to, so it is worth meeting all five before diving into any one.
...
Lattice & NTRU & FIPS 206 FN-DSA (draft) & signature \\
...
\bottomrule
\end{tabular}
\caption{The five families of post-quantum cryptography and where the standards sit. FIPS 203--206 implement the lattice and hash families; the code, multivariate, and isogeny families remain unstandardized but active, each a structurally different fallback.}
...
\begin{itemize}
  ...
  \item The answer is to rebuild public-key cryptography on five independent families of hard problems -- lattice, hash, code, multivariate, and isogeny -- the subject of the rest of this book, with FIPS 203--206 as the first standardized implementations.
\end{itemize}
