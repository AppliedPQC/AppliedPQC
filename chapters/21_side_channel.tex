...
touching the mathematics. This closing chapter is the book's final warning:
\emph{the math can be perfect and the deployment still broken.}
...
rather than a Gaussian (\ref{ch:cbd}), and why Falcon is the hardest of the three
signature standards to deploy safely (\ref{ch:fndsa}).
...
The total time a computation takes depends on the data it processed. If that data
...
power analysis} (DPA) is stronger: it collects many traces and correlates them,
...
The central software defense against the passive families -- timing, cache, power,
EM -- is \textbf{constant-time programming}. The name is slightly misleading: the
...
\begin{tikzpicture}[every node/.style={font=\scriptsize}]
  ...
  \node[font=\small\bfseries] at (3.1,0.1) {Constant-time (safe)};
...
\end{tikzpicture}
\caption{Top: a branch on a secret bit takes a short or a long path, so the running
time (and the power and cache trace) carries the secret. Bottom: the same conditional,
computed as one fixed sequence of operations ending in a masked select. The result is
identical, but nothing about the execution depends on the secret.}
\label{fig:ct-branch}
\end{figure}
...
arithmetic (variable-time on most hardware). This is exactly why ML-KEM and ML-DSA do
not use a Gaussian at all. They sample from the \emph{centered binomial distribution}
of Chapter~\ref{ch:cbd}, which is nothing but ``count the ones in two bit strings and
subtract'': no rejection, no tables, no floating point, and no data-dependent branch.
CBD is a security choice as much as an efficiency one. Falcon (\ref{ch:fndsa}) is the
cautionary opposite. Its trapdoor sampler genuinely needs a high-precision Gaussian
...
ML-DSA signing (\ref{ch:mldsa}) also loops: it draws a fresh mask, forms the response
$\mathbf{z}=\mathbf{y}+c\,\mathbf{s}_1$, and \emph{aborts} to retry whenever
$\mathbf{z}$ would leak the secret. The number of iterations is therefore variable --
but it depends on the freshly sampled mask, not on the secret key, so the abort count
is safe to reveal as public information. What must \emph{not} leak is the secret
inside the rejected attempts. FIPS 204 helps here by deriving the per-signature
randomness deterministically, as $\rho'' = H(K\,\|\,\mu)$ from the secret PRF key $K$
and the message: signing becomes reproducible, and its timing profile is a function of
public inputs rather than a fresh secret drawn on every call.
...
The Fujisaki--Okamoto (FO) transform closes this door with \textbf{implicit
...
ciphertext -- not an error. Both outcomes produce a key of identical form, and the
...
Constant-time coding defeats attacks that read a \emph{single} execution: timing,
cache, SPA. Differential power and EM analysis are stronger, because they average many
...
data -- the length of a message, or the public number of ML-DSA aborts -- is perfectly
...
This is the closing argument of the book, so it is worth stating plainly. The
...
And this is where the whole book closes the loop. The standards did not treat
implementation security as an afterthought; they engineered for it. CBD was chosen
over the Gaussian precisely because it is constant-time-friendly (\ref{ch:cbd});
ML-DSA derives its per-signature randomness deterministically so its abort loop leaks
nothing exploitable (\ref{ch:mldsa}); ML-KEM decapsulation is built on implicit
