...
\section{Why Kyber does not use Gaussian}

A very common question is: why does Kyber not use Gaussian noise? The short answer is engineering.

Discrete Gaussian sampling is expensive. In practice, implementations require methods such as:

\begin{itemize}
    \item cumulative distribution tables (CDT),
    \item Knuth--Yao sampling,
    \item Bernoulli sampling,
    \item rejection sampling.
\end{itemize}

These methods are mathematically elegant but costly in software and hardware. They may also introduce side-channel concerns. As a result, practical schemes such as Kyber prefer a simpler distribution.

\section{What is CBD?}

Kyber uses a centered binomial distribution (CBD). The main idea is very simple.

Take a small number of independent bits, for example $\eta$ bits. Count the number of ones in the first half and second half, then subtract them. The result is a small integer centered around zero. For example, if we sample $8$ bits and split them into two groups of $4$, then the difference of the two sums produces an integer in a small range. Repeating this many times gives a distribution that looks very much like a Gaussian.

This is why CBD is practical: it is easy to implement, fast to sample, and sufficient for the security and correctness goals of the scheme.

\section{Sage experiment: simulate CBD}

A toy implementation is easy:
...
samples = [cbd(3) for _ in range(10000)]
print(samples[:20])
\end{lstlisting}

If we plot the histogram, we observe that values near zero appear most often, while larger absolute values are rarer. This already resembles a bell-shaped distribution.

Algorithm~\ref{alg:cbd-sample} states this simulation formally, for a general parameter $\eta$; it is exactly what the Sage code above computes for $\eta=3$. A later chapter revisits CBD sampling in the precise form used by FIPS 203.

\begin{algorithm}[H]
\caption{Sample from CBD($\eta$)}
\label{alg:cbd-sample}
\begin{algorithmic}[1]
\Require Parameter $\eta \in \mathbb{Z}_{>0}$
\Ensure A sample $e \in \{-\eta,\dots,\eta\}$ drawn from the centered binomial distribution
\State Draw $2\eta$ independent uniform random bits $b_1,\dots,b_{2\eta} \in \{0,1\}$
\State $a \gets \sum_{i=1}^{\eta} b_i$ \Comment{sum of the first half of the bits}
\State $b \gets \sum_{i=\eta+1}^{2\eta} b_i$ \Comment{sum of the second half of the bits}
\State \Return $a - b$
\end{algorithmic}
\end{algorithm}
...
In later chapters, especially in RLWE and FHE, we will encounter the phrase \emph{noise growth}. This is because the error term grows when we add ciphertexts, multiply them, or perform more operations. Eventually, if the noise becomes too large, decryption fails. This is why noise management is one of the central practical challenges in modern cryptography.
